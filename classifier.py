...
class Classifier(object):
    ...
    def __init__(self, layers, layer_args, train_data, learning_rate, pn_ratio, res_dir):
        """ Assume same dimensions and neighborhoods for 'l_' and 'r_'
        ...
        self.dcnn = ('l_edge' not in train_data[0])
        self.in_vertex_dims = train_data[0]['l_vertex'].shape[-1]
        if self.dcnn:
            self.max_power = train_data[0]['l_power_series'].shape[1]
        else:
            self.in_edge_dims = train_data[0]['l_edge'].shape[-1]
            self.in_hood_size = train_data[0]['l_hood_indices'].shape[1]
        with self.graph.as_default():
            ...
            if self.dcnn:
                self.power_transition1 = tf.placeholder(tf.float32, [None, self.max_power, None],
                                                        name="power_transition_matrices")
                self.power_transition2 = tf.placeholder(tf.float32, [None, self.max_power, None],
                                                        name="power_transition_matrices")
            ...
            while i < len(layers):
                ...
                """
                if "merge" flag is in this layer, 
                then this is a merge layer and every subsequent layer is a merged layer.
                """
            ...
            # self.summary_writer = tf.summary.FileWriter(output_dir, self.graph)
